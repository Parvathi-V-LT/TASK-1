// TODO: should be cds/libx/queue/SchedulingService.js

const cds = require('../../lib')
const LOG = cds.log('scheduling')

const { registerTaskProcessor, OUTBOX_MESSAGES } = require('./utils.js')
...
module.exports = class SchedulingService extends cds.Service {
  constructor(...args) {
    ...
    if (cds.env.requires.multitenancy) {
      if (this.options.optimisticMarkers) {
    ...
    cds.on('served', async () => {
      const _flush = async tenant => {
        ...
        try {
          ...
          await cds.tx({ id, tenant }, () => cds.flush())
  ...
  init() {
    this.on(FLUSH, async msg => {
      ...
      await cds._with({ tenant: msg.data.tenant }, () => cds.flush(msg.data.target, opts))
    })
  }
  ...
  scheduleFlush({ target, tenant, flushOpts }) {
    const cb = () => {
      ...
      // REVISIT: why are we not using official cds.spawn()?
      if (tenant) cds._with({ tenant }, () => cds.flush(target, opts))
  ...
  _setupTaskProcessing({ queueName, context }) {
    if (!registerTaskProcessor(queueName, context)) return
  ...
  _get_marker({ queueName, tenant, after = 0 }) {
    const next_firing_time = _next_firing_time(Date.now() + after, this.marker_interval_ms, tenant)
    const timestamp = new Date(next_firing_time).toISOString()
    const ID = _deterministic_uuid(`${tenant}-${queueName}-${timestamp}`)
...
const _next_firing_time = (timestamp, interval, min_delta_fraction = 0.2, tenant) => {
...
function _deterministic_uuid(input) {
